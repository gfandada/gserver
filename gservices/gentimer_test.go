...
func Test_timer(t *testing.T) {
	...
	// 添加重复定时任务
	job1, ok := server.AddJobRepeat(time.Duration(time.Millisecond*20), 5, func() {
		fmt.Println("我是重复定时任务")
	})
	if !ok {
		t.Errorf("重复定时任务添加失败")
	}
	// 添加一次定时任务（适用于单次定时任务）
	job, ok := server.AddJobWithInterval(time.Duration(time.Second)*4, func() {
	...
	// 添加一个过期定时任务
	_, ok = server.AddJobWithDeadtime(time.Date(2016, 7, 4, 14, 16, 30, 0, time.Local), func() {
		fmt.Println("我是过期任务")
	})
	if ok {
