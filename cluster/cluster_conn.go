...
func (s *SessionMap) register(id uint64, v interface{}) {
	s.Lock()
	defer s.Unlock()
	s.records[id] = v
}

func (s *SessionMap) unregister(id uint64) {
	s.Lock()
	defer s.Unlock()
	delete(s.records, id)
}

func (s *SessionMap) query(id uint64) interface{} {
	s.RLock()
	defer s.RUnlock()
	return s.records[id]
}

func (s *SessionMap) count() int {
	s.RLock()
	defer s.RUnlock()
